...
Describe 'Confluence' {
    ...
    Context 'Integration' -Skip:(@($missingIntegrationVars).Count -gt 0) {
        ...
        Context 'Pages - CRUD lifecycle' {
            ...
            It 'Set-ConfluencePage updates the body and increments the version' {
                $newTitle = "$($script:prefix) Pages Root (updated)"
                $updated = Set-ConfluencePage -PageId $script:pagesRootId -Title $newTitle -Body '<p>Updated body.</p>' -Context 'ci'
